...
@section('content')
    ...
    <div class="wrapper wrapper__second wrapper__third">
        ...
        <section class="section__info">
            <div class="left">
                <h2 class="general__title">КАК СТАТЬ ЧАСТЬЮ НАШЕЙ КОМАНДЫ</h2>
            </div>
            <div class="right">
                <p class="general__subtitle">Начиная совместную работу с новым членом коллектива, мы прописываем стратегию развития и работы на несколько лет. Благодаря этому, каждый мастер имеет возможность чётко двигаться по намеченной траектории, постепенно поднимаясь вверх по карьерной лестнице.</p>
            </div>
        </section>
    </div>
        @livewire('vacancy')
...
@section('script')
    ...
    <script>
        {{--popup vacancy--}}
        $('.vacancy__tab-content .box').click(function () {
            $('.popup__vacancy').addClass('on');
            $('html').css('overflow', 'hidden')
        })
        $('.popup__vacancy-close').click(function () {
            $('.popup__vacancy').removeClass('on');
